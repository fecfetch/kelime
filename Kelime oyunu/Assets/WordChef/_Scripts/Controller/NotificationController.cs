...
public class NotificationController : MonoBehaviour {
    ...
    private void InGame()
    {
        ...
        if (num > 0)
        {
            Toast.instance.ShowMessage(string.Format("{0} bedava elmas kazandınız !", num));
        }
    ...
    private void PushUnlockLetter(char letter, double delay)
    {
        ...
        string message = "Sizin için " + letter + " harfini açtık. Haydi kaldığımız yerden devam edelim!";
        NotificationManager.SendWithAppIcon(TimeSpan.FromHours(delay), "Harfi aç", message, new Color(0, 0.6f, 1), NotificationIcon.Message);
#endif
    }
...
#if UNITY_ANDROID && !UNITY_EDITOR
        string message = " "+ num + " bedava elmas kazandınız. Oynamaya ve öğrenmeye devam :)";
