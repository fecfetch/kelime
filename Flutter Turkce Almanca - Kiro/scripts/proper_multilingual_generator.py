...
class ProperMultilingualLevelGenerator:
    ...
    def extract_word_entries_from_content(self, content: str) -> List[Dict]:
        ...
        # Find all word entries - look for the pattern: { 'word': '...', 'translations': { ... } }
        entry_pattern = r"\{\s*'word':\s*'([^']+)',\s*'translations':\s*\{([^}]+)\}\s*\}"
    ...
    def generate_levels(self, language_code: str, cefr_level: str, max_levels: int = 90) -> Dict:
        ...
        word_bank = self.load_combined_word_bank(language_code, cefr_level)
        
        if not word_bank:
            raise ValueError(f"No word bank loaded for {language_code} {cefr_level}")
        ...
        for entry in sorted_words:
            ...
            if len(available_targets) >= 2:  # Need at least 2 formable words
                ...
                multilingual_hints = self.generate_multilingual_hints(final_targets, word_bank)
        ...
        result = {
            "metadata": {
                ...
                "nativeLanguage": "tr",  # Default to Turkish as native
    ...
    def save_levels(self, levels_data: Dict, language_code: str, cefr_level: str):
        """Save levels to JSON file"""
        output_path = f"assets/levels/{language_code}_{cefr_level.lower()}_levels.json"
...
def main():
    ...
    parser.add_argument('--language', '-l', required=True, help='Language code (en, de)')
    ...
    try:
        levels_data = generator.generate_levels(args.language, args.level, args.max_levels)
        generator.save_levels(levels_data, args.language, args.level)
        print(f"✅ Successfully generated {args.language.upper()} {args.level} levels!")
