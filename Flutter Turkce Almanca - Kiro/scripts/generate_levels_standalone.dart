...
/// Main script execution - now supports multiple language pairs with organized output
Future<void> main(List<String> args) async {
  print('🚀 Starting organized level generation...');
  ...
  final Map<String, Map<String, List<Map<String, dynamic>>>> organizedLevels = {};
  ...
  for (final languagePair in languagePairs) {
    ...
    if (!organizedLevels.containsKey(targetLangCode)) {
      organizedLevels[targetLangCode] = {};
    }
    ...
    for (int world = 0; world < 7; world++) {
      ...
      // Initialize CEFR level structure
      if (!organizedLevels[targetLangCode]!.containsKey(cefrLevel)) {
        organizedLevels[targetLangCode]![cefrLevel] = [];
      }
        ...
        for (int level = 0; level < 18; level++) {
          ...
          try {
            ...
            if (levelData != null) {
              ...
              organizedLevels[targetLangCode]![cefrLevel]!.add(levelData);
              successfulLevels++;
  ...
  // Save levels in organized structure
  print('\n💾 Saving levels in organized structure...');
  
  for (final langEntry in organizedLevels.entries) {
    final targetLangCode = langEntry.key;
    final cefrGroups = langEntry.value;
    
    for (final cefrEntry in cefrGroups.entries) {
      final cefrLevel = cefrEntry.key;
      final levels = cefrEntry.value;
      
      if (levels.isEmpty) continue;
      
      // Sort levels by world, subWorld, and level for consistent ordering
      levels.sort((a, b) {
      ...
      // Get native language from first level
      final nativeLanguage = levels.first['nativeLanguage'] as String;
      
      final fileName = '${targetLangCode}_${cefrLevel.toLowerCase()}_levels.json';
      final cefrFile = File('assets/levels/$fileName');
      final cefrData = {
        'metadata': {
          ...
          'generatedAt': DateTime.now().toIso8601String(),
          'description': 'Levels for $cefrLevel proficiency in ${_getLanguageName(targetLangCode)}',
        },
        'levels': levels.map((level) => {
          'world': level['world'],
          'subWorld': level['subWorld'],
          'level': level['level'],
          'sourceWord': level['sourceWord'],
      ...
      await cefrFile.writeAsString(JsonEncoder.withIndent('  ').convert(cefrData));
      print('  📁 Saved ${levels.length} $cefrLevel levels → $fileName');
    }
  }
  
  // Generate organized index file
  print('\n📋 Generating organized index file...');
  await _generateOrganizedIndex();
  
  print('\n✅ Level generation completed!');
  print('📊 Summary:');
  print('   Total levels attempted: $totalLevels');
  print('   Successfully generated: $successfulLevels');
  print('   Success rate: ${(successfulLevels / totalLevels * 100).toStringAsFixed(1)}%');
  print('   Language pairs: ${languagePairs.length}');
  print('   Organized files created: ${organizedLevels.values.fold(0, (sum, cefrGroups) => sum + cefrGroups.length)}');
}
...
/// Generate organized index file that references the new structure
Future<void> _generateOrganizedIndex() async {
  final levelsDir = Directory('assets/levels');
  ...
  final organizedFiles = <Map<String, dynamic>>[];
  
  for (final file in files) {
    final fileName = file.path.split(Platform.pathSeparator).last;
    
    // Skip individual level files (old structure)
    if (fileName.startsWith('level_')) continue;
    ...
    try {
      ...
      print('⚠️ Warning: Could not process file $fileName: $e');
    }
  }
  
  // Sort by language and CEFR level
  organizedFiles.sort((a, b) {
    final langCompare = (a['targetLanguage'] as String).compareTo(b['targetLanguage'] as String);
    if (langCompare != 0) return langCompare;
    
    // Sort CEFR levels in order: A1, A2, B1, B2, C1, C2, MIXED
    final cefrOrder = ['A1', 'A2', 'B1', 'B2', 'C1', 'C2', 'MIXED'];
    final aIndex = cefrOrder.indexOf(a['cefrLevel'] as String);
    final bIndex = cefrOrder.indexOf(b['cefrLevel'] as String);
    return aIndex.compareTo(bIndex);
  });
  
  final indexData = {
    'version': '2.0',
    'structure': 'organized_by_cefr_level',
    'generatedAt': DateTime.now().toIso8601String(),
    'totalFiles': organizedFiles.length,
    'totalLevels': organizedFiles.fold<int>(0, (sum, file) => sum + (file['totalLevels'] as int)),
    'description': 'Organized level files grouped by target language and CEFR proficiency level',
  ...
  print('📋 Index file updated with ${organizedFiles.length} organized level files');
}